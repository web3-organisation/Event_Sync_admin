...
const SessionTitleCell = () => {
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
      ...
      >
        <CalendarMonthIcon sx={{ fontSize: 16, color: mode === "dark" ? "#A78BFA" : "#6C63FF" }} />
      </Box>
      <Box>
        <Typography sx={{ fontWeight: 700, color: mode === "dark" ? "#F1F5F9" : "#0F172A", fontSize: "0.88rem", lineHeight: 1.3 }}>
          {record.title as string}
        </Typography>
        {record.room && (
          <Typography sx={{ color: mode === "dark" ? "#475569" : "#64748B", fontSize: "0.72rem" }}>
...
const TimeRangeCell = () => {
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
      ...
      <Box>
        <Typography sx={{ fontWeight: 600, color: mode === "dark" ? "#E2E8F0" : "#0F172A", fontSize: "0.83rem" }}>
          {fmt(record.startTime as string)} → {fmt(record.endTime as string)}
        </Typography>
        <Typography sx={{ color: mode === "dark" ? "#475569" : "#64748B", fontSize: "0.72rem" }}>{day}</Typography>
      </Box>
    </Box>
  );
};
  ...
  if (now > end)
    return (
      <Chip
        label="Terminée"
        size="small"
        sx={{ background: mode === "dark" ? "rgba(71,85,105,0.15)" : "rgba(71,85,105,0.1)", color: mode === "dark" ? "#475569" : "#64748B", fontWeight: 600, fontSize: "0.72rem", height: 22, border: mode === "dark" ? "1px solid rgba(71,85,105,0.2)" : "1px solid rgba(71,85,105,0.1)" }}
      />
    );
  return (
    <Chip
      label="À venir"
      size="small"
      sx={{ background: "rgba(108,99,255,0.12)", color: mode === "dark" ? "#A78BFA" : "#6C63FF", fontWeight: 600, fontSize: "0.72rem", height: 22, border: "1px solid rgba(108,99,255,0.2)" }}
    />
  );
};
...
const SpeakersCell = () => {
  ...
  if (!Array.isArray(speakers) || !speakers.length)
    return <Typography sx={{ color: mode === "dark" ? "#374151" : "#94A3B8", fontSize: "0.78rem" }}>—</Typography>;
  return (
    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
      {speakers.map((name, i) => (
        <Box key={i} sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
          <Avatar sx={{ width: 20, height: 20, fontSize: "0.6rem", background: "linear-gradient(135deg, #6C63FF, #EC4899)" }}>
            {name[0]?.toUpperCase()}
          </Avatar>
          <Typography sx={{ color: mode === "dark" ? "#94A3B8" : "#64748B", fontSize: "0.78rem", fontWeight: 500 }}>{name}</Typography>
        </Box>
      ))}
    </Box>
  );
        ...
        <FunctionField
          ...
          render={() => (
            <Box sx={{ display: "flex", gap: 0.5, justifyContent: "flex-end" }}>
              <EditButton
                label=""
                sx={{ color: "#7C5CFF", minWidth: 0, p: "6px", borderRadius: "8px", "&:hover": { background: "rgba(124,92,255,0.12)" } }}
              />
              <DeleteButton
                label=""
                ...
                sx={{ color: "#EF4444", minWidth: 0, p: "6px", borderRadius: "8px", "&:hover": { background: "rgba(239,68,68,0.1)" } }}
              />
            </Box>
          )}
        />
